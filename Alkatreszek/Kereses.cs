...
namespace Alkatreszek
{
    internal class Kereses
    {
        ...
        public void Akcio()
        {
            ...
            foreach (var item in list)
            {
                if (item.Elso.ToString() == termek.ToLower() || termek.ToLower() == "összes" || termek.ToLower() =="egyik se") { VanTermek = true; }
            }

            if (termek !="egyik se" && VanTermek )
            {
                akcio =  IntInput("Hány zázalékkal: ");
                foreach (var i in list)
                {

                    if (termek == "összes")
                    {
                        i.Otodik = Convert.ToInt32(i.Otodik) - Convert.ToInt32(i.Otodik) / 100  * akcio;
